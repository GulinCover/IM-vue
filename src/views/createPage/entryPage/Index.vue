<template>
  <div class="create-entry-wrapper">
    <div class="title">
      <h4>{{locale.createEntryH3}}</h4>
      <p>{{locale.createEntryP}}</p>
    </div>

    <div class="create-content">
      <div class="name">
        <label>{{locale.entryName}}</label>
        <input v-model="entryName" type="text">
      </div>
      <div class="desc">
        <label>{{locale.description}}</label>
        <textarea v-model="entryDesc"></textarea>
      </div>
...
export default {
  ...
  watch: {
    contentName(newVal) {
      if (newVal === "" || this.entryName === "" || this.entryDesc === "") {
